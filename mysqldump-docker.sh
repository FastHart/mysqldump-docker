...
if [ 0 -eq $# ]
  then
    quit "Usage: $0 <container_name> <database>"
fi
CONTAINER=$1
DATABASE=$2
...
LockSet
OUT=$({ docker exec ${CONTAINER} /usr/bin/mysqldump -u ${DBUSER} --password=${DBPASS} ${DATABASE} | gzip - > ${BACKUPSTORE}/${FILENAME}; } 2>&1) || err "Unable to create dump: $OUT"
